...
//                               [--workers 3] [--format jpeg|png] [--crf 17] [--audio out/audio.wav] [--out out/showreel.mp4]
//
...
async function renderVideo(browser, port, options) {
  const from = Number(options.from ?? 0)
  const to = Number(options.to ?? DURATION)
  const subframes = Number(options.subframes ?? 4)
  ...
  await Promise.all(Array.from({ length: workers }, async (_, workerIndex) => {
    const chunkStart = firstFrame + workerIndex * framesPerWorker
    const chunkEnd = Math.min(lastFrame, chunkStart + framesPerWorker)
    if (chunkStart >= chunkEnd) return
    const chunkFile = join(scratch, `chunk-${String(workerIndex).padStart(2, '0')}.mkv`)
    chunkFiles[workerIndex] = chunkFile
    const { capture } = await openPage(browser, port, { solo: options.solo, scale })
    ...
    for (let frame = chunkStart; frame < chunkEnd; frame++) {
      ...
      if (framesDone % 30 === 0) {
        ...
        process.stdout.write(`\r${framesDone}/${totalFrames} frames · ${elapsed.toFixed(0)}s elapsed · ~${((elapsed / framesDone) * (totalFrames - framesDone)).toFixed(0)}s left   `)
      }
    }
    ffmpeg.stdin.end()
    await ffmpeg.done
  ...
  await writeFile(listFile, chunkFiles.filter(Boolean).map((file) => `file '${file}'`).join('\n'))
  ...
  console.log(`${out} (${totalFrames} frames × ${subframes} subframes in ${((Date.now() - startedAt) / 1000).toFixed(0)}s)`)
}
